...
**Не больше одного супервизора.** До перехода сервисов на один демон помощников и резолвер держат
procd (`init.d/steer`) на роутере и сервисы `steer supervise` и резолвера на телефоне. Демон с
`--supervise` рядом с ними поднял бы второй экземпляр каждого (второй обфускатор на том же порту,
второй резолвер на 5300). Поэтому `--supervise` включается только вместе со снятием прежних
экземпляров. Сам демон второго супервизора не обнаруживает.
...
`status` и `diag`, запущенные подкомандой мимо демона (rpcd на роутере), памяти демона не видят:
при `--supervise` поля `probe` у них не будет.
