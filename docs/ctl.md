...
| Спека перечитана | `apply`, `reload`, SIGHUP: убранные выходы — помощник гасится; новые — поднимается; у оставленных с новыми параметрами (узел подписки, сервер обфускации, файл xsteer, домен tgws, цель `via` и её метка, очередь, файл zapret и его содержимое) — перезапуск сразу; остальных не трогает; упавшим пауза сбрасывается |
...
| Резолвер | `steer dnsd --table-fd 3`: спеку не читает, таблицу доменных каналов демон пишет ему в трубу при запуске и после каждого перечитывания спеки — резолвер меняет её без перезапуска и перечитывает файлы списков |
