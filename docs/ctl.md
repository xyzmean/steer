...
состояние в памяти — `src/daemon/state.c`; стенд — `tests/ctlmatch.sh`.
...
SIGHUP. `status` и `explain` отвечают по ней, а всё, что меняется без спеки (устройство, выбранное
сторожем, `/sys`, счётчики nft, реестр меток), читают на каждый вызов. Спеку, заменённую в файле
...
прочиталась). Сторожу (`failover --loop`) сигнал не нужен: каждый его проход — новый процесс,
читающий спеку заново.
