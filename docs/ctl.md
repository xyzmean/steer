...
| Спека перечитана | `apply`, `reload`, SIGHUP: убранные выходы — помощник гасится; новые — поднимается; у оставленных с новыми параметрами (узел подписки, сервер обфускации, файл xsteer, домен tgws, цель `via` и её метка, очередь и файл zapret) — перезапуск сразу; остальных не трогает; упавшим пауза сбрасывается |
| Здоровье | помощник пишет события в трубу (`STEER_EVENT_FD`); демон держит по выходу состояние (поднят ли, причина отказа, узел, время) и шлёт подписчикам `helper-up`, `helper-down`, `node` |
