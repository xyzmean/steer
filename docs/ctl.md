...
команд: `apply`, пришедший по сокету сразу после старта, ждёт его, а первый проход сторожа
откладывается до конца применения. Итог — строкой в журнал демона (`steer[info] ctl: спека
...
отдаёт `status --fast` (раньше это делал отдельный экземпляр procd).
