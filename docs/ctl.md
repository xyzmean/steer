...
Код — `src/daemon/ctl.c` (в шапке те же правила с доводами), стенд — `tests/ctlmatch.sh`.
...
| `conns` | — | `steer conns`: соединения, которые движок повёл в свои выходы | 15 с |
| `dns-log` | — | `steer dns-log`: недавние имена резолвера и их каналы | 10 с |
| `put-file` | `<имя>`, тело — файл | положить файл списка в `/data/misc/steer/lists/<имя>` | — |
| `list-files` | — | что лежит в каталоге списков | — |
| `rm-file` | `<имя>` | убрать файл из каталога списков | — |
| `sub-check` | тело — файл подписки | разобрать подписку, ничего не сохраняя (`steer vless-nodes` по временному файлу) | 15 с |
