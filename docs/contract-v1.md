...
- **`probe`** — ход подъёма выхода `kind: vless`. Появляется **только когда `up: false`** и только
  ...
  Устройство туннеля vless создаётся **не сразу**: при `node: -1` («первый рабочий» — умолчание)
  клиент перебирает узлы подписки с таймаутом восемь секунд на узел, и устройство появляется только
  после выбора. Пока перебор идёт, `up: false` — то же самое значение, что при настоящем отказе. На
