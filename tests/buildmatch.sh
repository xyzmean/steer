...
bare=""
for f in src/steer.c src/failover.c src/dnsd.c src/obfs.c src/srs.c; do
...
check "голых «steer: » ровно столько, сколько отказов вызывающему" "src/steer.c:5 src/srs.c:23 " "$bare"
...
check "предупреждение об IPv6 в srs идёт с уровнем" "1" \
    "$(grep -c 'steer\[warn\] srs' src/srs.c)"
check "и голым «steer: » про IPv6 не пишется" "0" \
    "$(grep -c 'fprintf(stderr, "steer: srs: в наборе' src/srs.c)"
...
#
# Как это выглядело. src/srs.c появился 5 сентября: серверный список стенда его не получил, и
...
# том остановились. Через три дня то же с src/ext/certverify.c: tls13.c стал звать проверку
...
# с этим заголовком лежит .c, — этот .c обязан быть в том же списке. Файлы src/*.c считаются
# присутствующими всегда: они входят в каждую роль и в каждый стенд, а полноту их перечня
# держат проверки выше.
#
    ...
    while [ -n "$(printf '%s' "$_cm_work" | tr -d ' \n')" ]; do
        ...
        for _cm_f in $_cm_work; do
            ...
            for _cm_h in $(grep -oE '#include "[a-z0-9_]+\.h"' "$_cm_f" |
                           sed 's/.*"\(.*\)"/\1/'); do
                _cm_b="${_cm_h%.h}"
                for _cm_c in "$(dirname "$_cm_f")/$_cm_b.c" "src/$_cm_b.c" "src/ext/$_cm_b.c"; do
                    [ -f "$_cm_c" ] || continue
...
# отбрасываются: в шапке ext-test.sh пути упоминаются прозой («Включает src/ext/xshub.c»), а
...
for f in src/ext/xsclient.c src/ext/xshub.c; do
