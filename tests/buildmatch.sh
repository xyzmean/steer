...
bare=""
for f in src/daemon/steer.c src/daemon/failover.c src/dnsd/rules.c src/dnsd/wire.c src/dnsd/fakeip.c \
         src/dnsd/table.c src/dnsd/dlog.c src/dnsd/proxy.c src/dnsd/main.c src/lib/sindex.c \
         src/lib/nftnl.c src/lib/ctnl.c src/proto/obfs/obfs.c src/tools/srs.c; do
...
# steer.c: шесть законных строк — четыре отказа «этого нет в этой сборке» (VLESS, клиент
...
check "голых «steer: » ровно столько, сколько отказов вызывающему" "src/daemon/steer.c:6 src/tools/srs.c:23 " "$bare"
...
code_ids="$(grep -o 'diag("[a-z_]*"' src/daemon/steer.c | sed 's/diag("//;s/"//' | sort -u | tr '\n' ' ')"
