...
#
# srs.c: двадцать три строки, и все двадцать три — отказы РАЗОБРАТЬ набор. Три из них
# добавились вместе с третьим выходом (`--meta-out`): не создался файл сужения, он не
# записался до конца, и — главное — сужение есть, а печатать его некуда. Команда `srs-read` в этих
# случаях заканчивается кодом 1 или 2, а напечатанный список объявлен негодным к применению
...
check "предупреждение об IPv6 в srs идёт с уровнем" "1" \
    "$(grep -c 'steer\[warn\] srs' src/tools/srs.c)"
