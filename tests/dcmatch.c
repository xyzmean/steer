...
int main(void) {
    ...
    /* Медийные точки: у них своя точка kwsN-1, и спутать их нельзя. */
    eq("149.154.164.250", 4, 1, "медийный ДЦ узнан медийным");
    eq("149.154.167.151", 2, 1, "медийный второго ДЦ");
    /* Самый нагруженный адрес медийного ДЦ: на живом роутере он шёл как обычный, и
     * переписка работала, а картинки не грузились. */
    eq("149.154.167.222", 2, 1, "медийный второго ДЦ, основной адрес");
    eq("149.154.165.96",  4, 1, "медийный четвёртого ДЦ подсетью");
