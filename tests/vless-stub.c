 ...
 *
 * Собирать ради этого настоящую расширенную часть нельзя: он тянет mbedtls и docker (см. R-014).
...
/* Скачивание подписки — тоже расширенная часть: она разбирает узлы тем же кодом, что подъём
 * туннеля. diag её не зовёт.
 *
 * `sub-hwid` ЗАГЛУШКИ БОЛЬШЕ НЕ ИМЕЕТ: идентификатор переехал в src/tools/hwid.c, который входит в
 * обе сборки, и настоящая функция линкуется сюда сама. Заглушка рядом с ней означала бы два
 * определения одного имени — на этом сборка и споткнулась. */
...
/* Служебные подкоманды xsteer живут в src/proto/xsteer/xsadmin.c, и циклов в них нет вовсе — но
