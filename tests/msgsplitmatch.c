...
int main(void) {
    ...
    /* ОТКАЗ ОТ НАРЕЗКИ НЕ ИМЕЕТ ПРАВА ДУБЛИРОВАТЬ БАЙТЫ.
     *
     * Отступление переливает «остаток» потока как есть, и остаток обязан начинаться там, где
     * разбор остановился, — а не в начале куска, который разбирался. Иначе всё, что в этом
     * куске уже ушло наверх нарезанными кадрами, уходит второй раз следом: сервер получает
     * повтор, гамма обфускации у него уезжает, и сессия не «переливается как раньше», а
     * ломается насмерть. Это ровно тот случай, ради которого отступление и заведено, поэтому
     * проверяется он побайтово, а не по признаку «что-то дошло».
     *
     * Складываем нагрузку ВСЕХ кадров и сравниваем с потоком клиента: она обязана совпасть
     * ровно, без лишнего хвоста. */
    {
        struct cap c;
        cap_open(&c);
        struct upstream u;
        ...
        /* Два годных пакета и следом негодная длина — всё одним чтением, как приходит из TCP. */
        size_t k  = put_inter(s,     16, 0x71);
        k += put_inter(s + k, 24, 0x72);
        memset(s + k, 0, 8);                         /* длина 0 — так не бывает */
        k += 8;

        int r = ms_feed(&m, &u, s, k);
        cap_drain(&c);

        size_t lens[64], cat_n = 0;
        static unsigned char cat[262144];
        long fr = frames_parse(&c, lens, 64, cat, &cat_n);
        cap_close(&c);

        if (r < 0) bad("отступление не дублирует байты", "запись отказала", r, 0);
        else if (m.on) bad("отступление не дублирует байты", "нарезка осталась включённой", 1, 0);
        else if (fr < 0) bad("отступление не дублирует байты", "кадры не разбираются", (long)c.n, 0);
        else if (cat_n != k)
            bad("отступление не дублирует байты", "наверх ушло не столько байт", (long)cat_n, (long)k);
        else if (memcmp(cat, s, k) != 0)
            bad("отступление не дублирует байты", "содержимое не совпало", 0, 0);
        else ok("отступление не дублирует байты");
    }
