 ...
 * закрывает.
 *
 * ЧТО ИМЕННО ПРОВЕРЯЕТСЯ. У vless_connect семь путей выхода по ошибке, и каждый сам решает,
 * чем закрыться: одни зовут close(fd), другие vless_close(conn). Выбор не косметический —
