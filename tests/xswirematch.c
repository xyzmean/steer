...
int main(void) {
    ...
    /* ---- потолок рабочего MTU ----------------------------------------------
     * Одна функция на все входы: MTU чужого устройства, число из конфигурации и итог пробоя
     * пути. Предупреждать о числе больше предела клиент умел и раньше — не хватало того,
     * чтобы предупреждение чем-то заканчивалось. */
    {
        check("MTU: потолок это MTU туннеля при канале 1500", XS_MTU_DEF, xs_mtu_clamp(XS_MTU_DEF));
        check("MTU: ниже потолка проходит как есть", 1400, xs_mtu_clamp(1400));
        check("MTU: 576 проходит как есть", 576, xs_mtu_clamp(576));
        /* Обычная описка: в файле указан MTU КАНАЛА вместо MTU туннеля, и конфигурация её
         * принимает (проверка там стоит по XS_LINK_MAX). По этому числу считается предельный
         * сегмент, и 1500 давало 1521 байт — больше кадра канала. */
        check("MTU: MTU канала зажимается до MTU туннеля", XS_MTU_DEF, xs_mtu_clamp(XS_LINK_MAX));
        check("MTU: на байт выше потолка уже зажимается", XS_MTU_DEF, xs_mtu_clamp(XS_MTU_DEF + 1));
        /* Ноль и отрицательное значат «не выяснено» и дают потолок: с него начинает
         * согласование, а пробой пути опустит его, если путь уже. */
        check("MTU: ноль это «не выяснено» и даёт потолок", XS_MTU_DEF, xs_mtu_clamp(0));
        check("MTU: отрицательное тоже даёт потолок", XS_MTU_DEF, xs_mtu_clamp(-1));
        /* Предельный сегмент, посчитанный по зажатому числу, обязан влезать в кадр канала —
         * ровно то свойство, ради которого потолок и нужен. */
        check("MTU: предельный сегмент влезает в кадр канала", 1,
              xs_mtu_clamp(XS_LINK_MAX) + XS_OVERHEAD - 40 <= XS_LINK_MAX);
    }
    ...
    /* ---- заголовок записи -------------------------------------------------- */
    {
        ...
        /* Предел ФОРМАТА, а не поля: тело ровно XS_MAX_RECORD ещё законно, на байт больше — уже
         * нет. Иначе набор кадров, посчитанный без тега, доезжает до провода, а приёмник рвёт
         * соединение по чужой длине. */
        check("запись: тело ровно предела — принято", 0, xs_rec_build(h, XS_MAX_RECORD));
        check("запись: тело на байт больше предела — отказ", -1,
              xs_rec_build(h, XS_MAX_RECORD + 1));
        check("запись: открытый текст плюс тег влезает в предел", 0,
              xs_rec_build(h, XS_MAX_PLAIN + XS_TAG));
    }
    {
     ...
     * битого контейнера ЦЕЛИКОМ и — главное — что пачка дешевле тех же кадров поодиночке. */
    {
        ...
        /* Предел ОТКРЫТОГО текста: буфер тут ёмкостью в целую запись, поэтому «не влезло по
         * буферу» не сработает, а мерить надо XS_MAX_PLAIN — тело записи это открытый текст
         * ВМЕСТЕ с тегом. Ровно этот рубеж поймал бы потерянное слагаемое `+ XS_TAG` в порте. */
        static uint8_t wide[XS_MAX_RECORD];
        struct xs_frame two[2];
        two[0].p = wide; two[0].n = XS_MAX_PLAIN - XS_BATCH_HDR - 2 - 2 - 1;
        two[1].p = wide; two[1].n = 1;
        static uint8_t big[XS_MAX_RECORD + XS_TAG];
        check("пачка: открытый текст ровно предела собирается", (long)XS_MAX_PLAIN,
              (long)xs_batch_build(big, sizeof(big), two, 2));
        two[1].n = 2;
        check("пачка: открытый текст на байт больше предела — отказ", 0,
              (long)xs_batch_build(big, sizeof(big), two, 2));
