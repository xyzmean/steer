...
/* Индекс занятого слота пары; -1 — такого нет. Тот же порядок обхода, что у warm_take. */
static int slot_of(short dc, short media) {
    for (size_t i = 0; i < WARM_SLOTS; i++)
        if (g_warm[i].busy && g_warm[i].dc == dc && g_warm[i].media == media) return (int)i;
    return -1;
}
...
int main(void) {
    ...
    int given = slot_of(2, 0);
    eq("готовое соединение отдаётся клиенту", warm_take(2, 0, &u, sni, sizeof(sni)), 1);
    eq("отданное из запаса убрано", busy_for(2, 0), 1);
    /* Владелец соединения ровно один. Пока читатели слотов сверяются с busy, копия
     * дескриптора в отданном слоте недостижима — но это заряженный ствол: любой второй
     * читатель (второй наполнитель, сборщик статистики, слив запаса при остановке) закроет
     * чужой дескриптор и освободит уже освобождённый контекст AEAD. */
    eq("отданный слот не хранит дескриптора", g_warm[given].u.fd, -1);
    eq("отданный слот не хранит признака TLS", g_warm[given].u.tls_on, 0);
    close(u.fd);
