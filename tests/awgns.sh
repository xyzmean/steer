...
# при каждой починке. Теперь замеры живут в памяти процесса круга (проход — автомат на его цикле
# событий, в том же процессе; имя Endpoint при починке разрешает рабочий поток), остальное
# пишется только при изменении. Проверяется и то, что память
