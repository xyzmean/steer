...
int main(void) {
    ...
    {
        /* Повторная вставка ПОВЕРХ надгробия, лежащего раньше ключа в цепочке проб.
         *
         * Случайный прогон ниже этого не ловит и не может: он вставляет только тогда, когда
         * записи нет (`!live[i]`), то есть ровно в том порядке, в каком её сейчас зовёт хаб.
         * А контракт у функции шире — «повторная вставка обновляет» проверяется выше и на
         * этом же ключе обязано выполняться независимо от того, что лежит в цепочке до него.
         *
         * Раскладка строится из пары ключей с одним хешем: A садится в ячейку h, K — в h+1.
         * После удаления A на h остаётся надгробие, и повторная вставка K видит его раньше,
         * чем свою живую запись. */
        struct xs_sidx x;
        xs_sidx_reset(&x);
        uint32_t ka = ip4(10, 0, 0, 1);
        uint16_t pa = 1000;
        uint32_t kk = 0;
        uint16_t pk = 0;
        for (uint32_t a = ip4(10, 0, 0, 2); a < ip4(10, 1, 0, 0) && !kk; a++)
            for (uint16_t p = 1000; p < 1200; p++)
                if (xs_sess_hash(a, p) == xs_sess_hash(ka, pa)) { kk = a; pk = p; break; }
        check("индекс: пара с одним хешем найдена", 1, kk != 0);

        xs_sidx_insert(&x, ka, pa, 11);          /* A -> h   */
        xs_sidx_insert(&x, kk, pk, 22);          /* K -> h+1 */
        xs_sidx_remove(&x, ka, pa);              /* на h — надгробие */
        check("индекс: надгробие перед ключом — K ещё находится", 22, xs_sidx_find(&x, kk, pk));

        check("индекс: повторная вставка поверх надгробия", 0, xs_sidx_insert(&x, kk, pk, 33));
        check("индекс: обновлённое значение за надгробием", 33, xs_sidx_find(&x, kk, pk));
        /* Главное: запись ОДНА. Второй экземпляр того же ключа виден не сразу — find
         * возвращает первый, — а после удаления: сессия закрыта, а индекс отдаёт номер
         * СТАРОЙ, давно освобождённой записи. Тот же класс, что перепутывание объекта по
         * номеру дескриптора. */
        check("индекс: дубликата ключа не появилось", 1, (long)x.used);
        xs_sidx_remove(&x, kk, pk);
        check("индекс: после удаления ключа его нет", -1, xs_sidx_find(&x, kk, pk));
    }
    {
