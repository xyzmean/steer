...
$CC -O1 -w $STEER_INC $MBED_INC "$PRIV" -DSTEER_SERVER -o "$BUILD/steer-hub-native" \
	src/lib/run.c src/lib/jsonw.c src/compile/groups.c src/compile/generate.c src/daemon/fwcheck.c src/daemon/apply.c src/daemon/status.c src/daemon/nftquery.c src/daemon/diag.c src/daemon/explain.c src/daemon/supervise.c src/daemon/watch.c src/daemon/main.c src/lib/jsonr.c src/lib/tmpfile.c src/model/parse.c src/model/registry.c src/model/probe.c src/compile/nftcompat.c src/lib/sindex.c src/lib/nftnl.c src/lib/ctnl.c src/dnsd/rules.c src/dnsd/wire.c src/dnsd/fakeip.c src/dnsd/table.c src/dnsd/dlog.c src/dnsd/proxy.c src/dnsd/main.c src/daemon/failover.c src/tools/aggregate.c src/proto/obfs/obfs.c src/cli/cli.c \
