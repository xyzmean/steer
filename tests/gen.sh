...
#
# apply заменяет таблицу новой, а счётчики живут в правилах — значит без переноса
...
# быть отказом (гибридные списки — резолвер берёт из того же файла доменные строки, если у
# группы есть доменный набор), но
...
# Набор без единого адреса объявляется БЕЗ строки elements: `elements = {  }` nft не
# принимает и отвергает весь набор правил, то есть dry-run выше был зелёным, а настоящий
# apply на таком списке снимал бы маршрутизацию целиком.
check "пустой набор без пустой строки elements" "0" \
    "$("$BIN" apply --dry-run --spec "$tmp/dashspec.json" --state-dir "$tmp/state-dash" 2>/dev/null |
       grep -c 'elements = {  *}')"
# И человеку не обещают «будет работать по именам», когда доменного набора у канала нет:
# имена из адресного файла резолвер берёт только в доменной группе.
check "без доменного соседа сказано, что канал ничего не поймает" "1" \
    "$("$BIN" apply --dry-run --spec "$tmp/dashspec.json" --state-dir "$tmp/state-dash" 2>&1 |
       grep -c 'ничего не поймает')"
...
if [ -f "$init" ]; then
    ...
    # молчал (см. выше).
    #
    # С шага 6 (docs/architecture.md, раздел 4а) спрашивать движку больше некому: резолвер,
    # сторож и помощники — дети демона, и решает о них демон своим кодом (needs-dnsd — то же
    # dnsd_wanted у супервизора). Прежняя ловушка — сторож, не запущенный на конфигурации без
    # доменов, — теперь закрыта тем, что экземпляр один и сторож у него всегда (--watch).
    check "init script does not grep the spec for keys" "0" \
        "$(grep -c "grep -q '\"domains" "$init")"
    check "init-скрипт держит один экземпляр — демон со сторожем и супервизором" "1 1" \
        "$(grep -c '^[^#]*procd_open_instance' "$init") $(grep -c '^[^#]*daemon --watch --supervise' "$init")"
    check "  ни сторожа, ни резолвера отдельными экземплярами" "0" \
        "$(grep -cE '^[^#]*(steer failover|steer dnsd|procd_open_instance (failover|dnsd))' "$init")"
fi
...
# Выход узнаётся по метке СОЕДИНЕНИЯ: метку пакета между маршрутизацией и postrouting
# переписывает Tailscale (`mark and 0xff00ffff xor 0x40000` на всём, что пришло с tailscale0),
# и по ней первые четыре выхода до очереди не доезжали — с телефона через Tailscale обход не
# работал, из LAN работал. ct mark стоит с prerouting и чужими не трогается.
check "выход узнаётся по ct mark, а не по метке пакета" "2" \
    "$(printf '%s\n' "$zout" | grep -c 'ct mark and 0x0ff00000 == 0x00[12]00000 ct original packets 1-9 meta mark set')"
check "по метке пакета очередь больше не ищется" "0" \
    "$(printf '%s\n' "$zout" | grep -c 'meta mark and 0x0ff00000 == 0x00[12]00000 ct original')"
...
# ТРИ правила, а не четыре: правила zapret про «postnat traffic» здесь быть не должно.
#
# У zapret бит 0x20000000 значит «пакет взят из цепочки ПОСЛЕ трансляции адресов»; у нас тот же
# бит значит «пакет нашего обработчика». Правило мы взяли вместе с чужим смыслом бита — и стали
# снимать с учёта ВСЕ пакеты нашего nfqws. Снятый учёт означает, что к ним не применяется
# трансляция адресов, а копии nfqws строит из исходного кортежа соединения: они уходят в
# интернет с частным адресом клиента, сервер их не узнаёт, канал виснет до таймаута.
#
# Проверено на стенде в QEMU с клиентом за LAN: с этим правилом — таймаут 5 проб из 5, без него
# — HTTP 200 за 19-40 мс, 4 из 4. Тот же nfqws и та же стратегия на весь роутер службой zapret
# работают в обоих случаях, то есть дело именно в правиле.
check "три правила notrack: фрагменты и данные без ACK, но не «postnat»" "3" \
...
# заново через свою паузу (respawn 3600 5 0). Заворот `udp dport 53 -> :5300` в это время
# стоит, слушать некому — и ВСЯ локальная сеть получает «port unreachable» на каждый запрос
# имени. Замерено на стенде в QEMU: ровно 5 секунд после КАЖДОГО «Применить», включая ночное
# обновление списков. По HUP резолвер перечитывает списки, не теряя запросов, но состава
# каналов не пересобирает — значит различать два случая обязан движок, а не init-скрипт.
#
# Здесь проверяется ровно то, на что опирается init-скрипт: подпись устойчива к повторному
# вызову и МЕНЯЕТСЯ, когда меняется то, чего HUP не умеет.
# Спека с доменным каналом — та же, на которой проверяется весь доменный путь ниже.
sig1="$("$BIN" dnsd-sig --spec "$tmp/dspec.json" --state-dir "$tmp/state-sig" 2>/dev/null)"
...
# его не пересоберёт. Подпись обязана разойтись, иначе мы пошлём HUP там, где нужен
# перезапуск, и человек получит старые каналы при новых правилах.
sed 's#"domains_file": "[^"]*"#"domains_file": "'"$tmp"'/other.lst"#' "$tmp/dspec.json" > "$tmp/dspec-other.json"
printf 'other.example\n' > "$tmp/other.lst"
sig3="$("$BIN" dnsd-sig --spec "$tmp/dspec-other.json" --state-dir "$tmp/state-sig" 2>/dev/null)"
check "смена файла списка меняет подпись" "no" \
    "$([ "$sig1" = "$sig3" ] && echo yes || echo no)"
# И резолвер кладёт ТУ ЖЕ подпись в каталог состояния — иначе сравнивать нечего. Запускать
# его здесь нельзя (нужна сеть и netlink), поэтому проверяется шов: команда обязана
# принимать --state-dir, иначе стенд писал бы подпись в /var/lib настоящей системы.
# Две строки: сама строка использования и описание флага в перечне ниже.
check "команда объявлена с --state-dir" "2" \
    "$("$BIN" help dnsd-sig 2>&1 | grep -c 'state-dir')"
...
if [ -f "$init" ]; then
    ...
    #
    # С шага 6 обработчики поднимает демон (src/kinds/zapret.c, zapret_helper): номер очереди он
    # берёт тем же out_zapret_queue, что и zapret-instances, а init-скрипт их не поднимает вовсе.
    check "обработчики zapret поднимает демон, а не init-скрипт" "0 1" \
        "$(grep -c '^[^#]*zapret-instances\|^[^#]*procd_open_instance "zapret_' "$init") $(grep -c '^static int zapret_helper' src/kinds/zapret.c)"
    ...
    check "обработчик поднимается обёрткой" "1" \
        "$(grep -c '"steer-nfqws"' src/kinds/zapret.c)"
...
if [ -f "$wrap" ]; then
    ...
    # пока процесса нет, помеченный трафик отбрасывается ядром. С шага 6 порядок держит демон:
