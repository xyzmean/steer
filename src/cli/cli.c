...
{"daemon", "Маршрутизация", "",
 ...
 "открытым и шлёт события (спека применена или не прочитана). На Android это дверь\n"
 "приложения splify2 к движку: политика не даёт ему ни исполнить движок, ни прочитать\n"
 "его файлы. Пускает root, system и процессы домена SELinux из --allow-domain;\n"
 "протокол — в docs/ctl.md. Работает на переднем плане, процессом управляет init.",
 "", 0, 1, 0, 0, 0},
