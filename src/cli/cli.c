...
{"ctl-serve", "Маршрутизация", "",
 "управляющий сокет: status, diag, apply и прочее для приложения",
 "Слушает unix-сокет и исполняет по запросу короткий список команд — status, diag,\n"
 "explain, apply и check со спекой в теле запроса, reload, vless-nodes, vless-probe,\n"
 "conns, dns-log, sub-check и файлы списков (put-file, list-files, rm-file).\n"
 "Нужен там, где управляющий слой не может звать движок как программу: на Android\n"
 "приложению splify2 политика не даёт ни исполнить движок, ни прочитать его файлы.\n"
 "Пускает root, system и процессы домена SELinux из --allow-domain; протокол — в\n"
 "docs/ctl.md. Работает на переднем плане, процессом управляет init.",
 "", 0, 1, 0, 0, 0},
