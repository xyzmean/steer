 ...
 * (src/ext/xsclient.c, state_write), и шапка там прямо говорит, что файл этот — для
 ...
 *
 * ЧЕМ. Инструментом conntrack (пакет conntrack). Его может не быть — тогда предупреждаем
 * ОДИН раз за процесс и работаем как раньше: маршрутизация верна для новых соединений, а
 * установленные доживают свой век. Делать зависимость обязательной незачем: на роутере без
 * выгрузки потоков и без долгих соединений разница незаметна. */
void conntrack_evict(unsigned mark) {
    ...
    warned = 1;
    fprintf(stderr, LOG_W "инструмента conntrack нет: смена маршрута выхода не снимает уже "
                    "установленные соединения. С включённой выгрузкой потоков "
                    "(flow_offloading) они продолжат идти прежним путём — поставьте пакет "
                    "conntrack\n");
}
