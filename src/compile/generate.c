 ...
 *
 * Части, которые зависят от вида выхода, собраны в функции-построители с сигнатурой
 * `(struct nft_rs *, const struct spec *, const struct output *)` — nft_emit_zapret и
 * nft_emit_tgws; это будущий kind_ops.emit модулей видов (docs/architecture.md, «Вид
 * выхода»). Части, которые есть только на телефоне (разметка на выходе, счёт скачанного на
 * входе, заворот DNS приложений, «кто» по владельцу сокета), — nft_emit_output_mark,
 * nft_emit_output_dns и local_who; их заберёт platform_ops. */
#define _GNU_SOURCE
#include <stdarg.h>
#include <stdio.h>
#include <stdlib.h>
 ...
 *
 * ПОСТРОИТЕЛЬ — НА ОДИН ВЫХОД (будущий kind_ops.emit): цепочки общие для всех выходов
 ...
 * уходит только в таблицу ip. Построитель — на один выход, цепочку заводит первый вызов. */
...
int nft_build(struct nft_rs *rs, const struct spec *sp, const struct groups *gr,
    ...
    /* Построители видов. Двумя проходами, а не одним: все цепочки zapret в тексте стоят
     * раньше цепочки моста, в каком бы порядке выходы ни шли в спеке. */
    for (size_t i = 0; i < sp->out_n; i++)
        if (sp->out[i].kind == OUT_ZAPRET) nft_emit_zapret(rs, sp, &sp->out[i]);
    for (size_t i = 0; i < sp->out_n; i++)
        if (sp->out[i].kind == OUT_TGWS) nft_emit_tgws(rs, sp, &sp->out[i]);
#ifndef STEER_TGWS
