...
int xs_conn_on_seg(struct xs_conn *c, const struct obfs_seg *s, long long now) {
     ...
     *
     * RST здесь означает ровно одно: у той стороны нашей сессии НЕТ. Хаб, у которого сессия
     * ушла по простою или который перезапустился, отвечает им на каждый наш сегмент. Номер в
     * таком RST не тот, которого мы ждём, поэтому сессию он не снимает (см. ниже) — и это
     * правильно. Но пока он обновлял время приёма, чужой отказ ДОКАЗЫВАЛ живость пути: порог
     * тишины не достигался никогда, и пир сидел на соединении, которое не несёт ни байта, пока
     * его не пересоздаст смена ключей. На живом роутере это тринадцать минут без единой строки
     * в журнале: проба пути не получала эха ни на одном размере, а туннель «работал».
     *
     * Поэтому RST не обновляет ни время приёма, ни запас незанятости — ни принятый, ни
     * отвергнутый. Принятому это безразлично (он снимает сессию), а отвергнутый перестаёт быть
     * признаком жизни и остаётся тем, что он есть: отказом, после которого путь молчит. */
    if (!(s->flags & TH_RST)) {
        c->last_rx = now;
        c->stall = 0;
    }
    /* RST принимается только с номером, которого мы ЖДЁМ, — то же правило, что у настоящего
     * стека после RFC 5961. Четвёрка наблюдаема на всём пути, адрес источника подделывается
     * свободно, а аутентификации на служебном сегменте нет по построению: обфускатор
     * притворяется TCP, а не защищает его. Пока номер не спрашивался, один пакет с угаданной
     * четвёркой рвал живой туннель, и повторять его можно было сколько угодно.
     *
     * Почему именно РАВЕНСТВО, а не «в пределах окна»: окно мы объявляем с масштабом, то есть
     * 8 МиБ (см. next_win), и «в окне» стоило бы слепому 2^32/8 МиБ ≈ 512 попыток. Равенство
     * стоит 2^32.
     *
     * Законный RST от этого не страдает, и он один — хаб не отвечает на порту (упал, переехал,
     * порт закрыт правилом). Ядро той стороны отвечает RST'ом, номер которого равен полю
     ...
     * него ещё нет, и проверять в этой фазе можно только его.
     *
     * Отвергнутый RST не значит «висеть вечно»: путь, который действительно умер, всё равно
     * объявляется мёртвым по тишине через XSC_DEAD_MS. */
    if (s->flags & TH_RST) {
        if (c->state == XSC_SYN_SENT) {
            if (!(s->flags & TH_ACK) || s->ack != c->isn_tx + 1) return 0;
        } else if (s->seq != c->ack) {
            return 0;
        }
        return -1;
