...
size_t xs_hs_alert(uint8_t *out, size_t cap) {
     ...
     * которому предложили то, чего он не может. Само по себе это от активного зондирования не
     * спасает: сервер с подходящим сертификатом ответил бы ServerHello и сертификатом, а не
     * отказом. Молчание было бы отличимо ещё сильнее, поэтому оповещение — ответ по умолчанию,
     * но лишь один из четырёх режимов (`Decoy` = alert|silent|reset|proxy): подлинный ответ
     * прибору даёт режим `proxy` у хаба, а не эта функция (см. src/proto/xsteer/xshub.c, дорожка
     * PH_PROXY, и docs/xsteer.md). */
