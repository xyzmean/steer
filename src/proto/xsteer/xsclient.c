...
static void probe_done(struct spoke *s, const char *dev, long long now) {
    ...
    if (s->p_lo <= XS_MTU_FLOOR) {
        ...
        s->mtu_confirmed = 0;
        /* И ВСЕМ СОЕДИНЕНИЯМ. Кадр ниже уходит только по этому, нулевому; остальные называют хабу
         * g_mtu_pub, и только когда он сменился. Без этой строки их сессии на хабе оставались с
         * прежним размером — та же дыра, что закрыта в Go-клиенте (I-278). Низ, а не ноль: при
         * нуле остальные не назвали бы ничего. */
        g_mtu_pub = XS_MTU_FLOOR;
...
static int stream_dial(struct spoke *s) {
     ...
     * prerouting по адресу ИСТОЧНИКА из локальной сети (см. src/compile/generate.c); пакеты, рождённые
     * самим роутером, prerouting не проходят вовсе, поэтому метки не получают и в туннель не
     * попадают — даже при полном туннеле 0.0.0.0/0. Ни привязки к устройству, ни обхода /32 к
     * адресу хаба здесь поэтому нет. На Windows у реализации на Go всё иначе (там маршрутами
     * владеет система, и сокет приходится привязывать через IP_UNICAST_IF), и разница между
     * платформами объяснена там же, в tun/pin_other.go.
     *
     * SO_MARK же ставится, но не ради исключения, а ради `via`: метка выхода-цели уводит это
     * соединение в его устройство (см. «вложенные выходы» в spec.h). До connect() — маршрут
     * выбирается там. Без via метка нулевая на роутере и «сам движок» на телефоне. */
    if (obfs_mark_sock(fd) != 0) { close(fd); return -1; }
