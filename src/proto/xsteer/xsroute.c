...
int xs_sidx_insert(struct xs_sidx *x, uint32_t addr, uint16_t port, int idx) {
    ...
    unsigned h = xs_sess_hash(addr, port);
    /* Первое встреченное надгробие запоминается, но занимается НЕ СРАЗУ: за ним по цепочке
     * может лежать живая запись этого же ключа, положенная туда, когда надгробия ещё не
     * было. Сесть в надгробие, не досмотрев цепочку до пустой ячейки, значит завести ВТОРОЙ
     * экземпляр ключа. Виден он не сразу — поиск возвращает первый, — а после удаления:
     * xs_sidx_remove гасит только первый, и закрытая сессия продолжает находиться, отдавая
     * номер давно освобождённой записи. Тот же класс, что перепутывание объекта по номеру
     * дескриптора, и воспроизводится только при совпадении хешей у соседей. */
    struct xs_sidx_ent *grave = NULL;
    for (unsigned step = 0; step < XS_SIDX_SLOTS; step++) {
        struct xs_sidx_ent *e = &x->slot[(h + step) & (XS_SIDX_SLOTS - 1)];
        if (e->state == 1) {
            if (e->addr == addr && e->port == port) { e->idx = (int16_t)idx; return 0; }
            continue;
        }
        if (e->state == 2) { if (!grave) grave = e; continue; }
        /* Пустая ячейка: дальше этого ключа в таблице быть не может — поиск обрывается на
         * ней так же. Значит запись новая, и место ей — в самом раннем надгробии цепочки
         * (короче проба), а если надгробий не было, здесь же. */
        if (grave) { e = grave; x->dead--; }
        e->addr = addr;
        e->port = port;
        e->idx = (int16_t)idx;
        e->state = 1;
    ...
    /* Досюда не дойти: страж выше оставляет в таблице не меньше двух пустых ячеек, а цикл
     * останавливается на первой же. Отказ здесь — на случай, если страж когда-нибудь
     * ослабят. */
    return -1;
}
