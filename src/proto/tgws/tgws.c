 ...
 *
 * СЕРТИФИКАТ МОСТ НЕ ПРОВЕРЯЕТ, и это осознанно. Проверка цепочки в сборке есть
 * (certverify.c, её зовёт клиент security=tls), но мосту она ничего не даёт: внутри едет
 * MTProto, у которого своя сквозная аутентификация — ключ согласуется по DH и подписан
 * ключами Telegram, зашитыми в приложение. Посредник во внешнем TLS не прочитает и не
 * подделает ни одного сообщения — ему остаётся только оборвать соединение, что он и так
 * может. Здесь мост РАСХОДИТСЯ с tg-ws-proxy: нынешний tg-ws-proxy цепочку проверяет — по
 * корням certifi (proxy/utils.py, create_ssl_context), а имя в сертификате не сверяет
 * только тогда, когда сам подменяет SNI.
 *
...
/* Домен точек веб-сокета — тот, что назван выходу в спеке (`domain`); это умолчание
 * действует, только когда его не назвали ни спека, ни STEER_TGWS_DOMAIN.
 *
 * САМ web.telegram.org МОСТ ПРОБУЕТ ВСЕГДА, но только для ДЦ2 и ДЦ4 — см. «прямо к
 * Telegram» ниже. Точки kwsN.web.telegram.org отвечают только TLS 1.2 (сертификат GoDaddy,
 * ECDHE-RSA-AES128-GCM-SHA256), поэтому у этого пути своё рукопожатие (tls12_handshake) и
 * свой адрес 149.154.167.220; провайдеры, режущие MTProto к адресам дата-центров, его
 * обычно пропускают. Прежняя запись здесь («прямой не работает: у нас только TLS 1.3, а
 * адреса режут») устарела вместе с появлением этого пути.
 *
 * ОСТАЛЬНЫМ НУЖЕН ДОМЕН ЗА CLOUDFLARE. На адресе веб-клиента kws1/kws3/kws5 не отвечают, и
 * ДЦ203 там не обслуживается, поэтому они идут через `kwsN.<домен>` с записью на веб-точку
 * Telegram и включённым проксированием: такой домен отвечает TLS 1.3 с адресов Cloudflare,
 * блокировать которые дорого. Домен задаётся выходу в спеке, потому что он ЧУЖОЙ: это либо
 * домен самого владельца роутера, либо тот, который кто-то держит для сообщества, и
 * зашивать его в движок значило бы решать за человека, чьим каналом он пользуется.
 *
 * Умолчанием пользуется проба (tgws-probe без STEER_TGWS_DOMAIN): она говорит только
 * TLS 1.3 и ходит по имени, так что против самого web.telegram.org она упрётся в отказ TLS —
 * проверять ею надо домен за Cloudflare. */
...
static void dc_table_init(void) {
    ...
    if (!path) path = plat_etc_path(dflt, sizeof dflt, "tgws-dc.conf");
    FILE *f = fopen(path, "r");
    if (!f) return;
    char line[128];
    size_t added = 0;
...
/* Потолок ВЫШЕ размера пула, а не вровень с ним. Стояло 16 при 21 домене в
 * data/tgws-domains.lst — чтение в alt_init обрывается по этому числу, и последние пять
 * строк файла мост не видел никогда. Снаружи это неотличимо от «домены кончились»:
 * пул исчерпан, а пять живых точек лежат в файле непрочитанными. */
#define MAX_ALT 24
...
static void alt_init(void) {
    ...
    if (!path) path = plat_etc_path(dflt, sizeof dflt, "tgws-domains.lst");
    FILE *f = fopen(path, "r");
    if (!f) return;
...
static void route_init(void) {
    ...
    if (!path) path = plat_etc_path(dflt, sizeof dflt, "tgws-route.conf");
    FILE *f = fopen(path, "r");
    if (!f) return;
    char line[224];
    size_t n = 0;
 ...
 *
 * МЕДИЙНЫЙ ДЦ БЕЗ СВОЕЙ ЗАПИСИ БЕРЁТ ДОМЕН ОБЫЧНОГО ДЦ С ТЕМ ЖЕ НОМЕРОМ, и это правка
 * самой массовой жалобы выборки — «картинки и файлы в Telegram не грузятся».
 *
 * Подбор обещает каждому дата-центру свой домен, но медийную точку он меряет только у ДЦ2
 * (см. brb/tgws, шаг подбора): у медийных ДЦ 1, 3, 4, 5 строки в карте не появляется вовсе.
 * Запасного хода здесь не было, route_domain возвращал NULL, и мост откатывался к общему
 * домену — то есть ровно к поведению ДО починки, только теперь на самом объёмном трафике.
 * Это же объясняет асимметрию «на ПК стабильно, на телефоне ни в какую»: телефон чаще ходит
 * в чужой медийный ДЦ.
 *
 * Почему сосед по номеру — законная замена. Домен здесь не принадлежит дата-центру: это
 * чужая точка Cloudflare, через которую мост разговаривает с Telegram, и измеряли у неё
 * пригодность края, а не близость к ДЦ. Домен, который подошёл ДЦ4, подходит и его
 * медийной половине — он у того же провайдера и в той же сети. Полноценный замер по всем
 * пяти медийным ДЦ лучше и делается в подборе; здесь — дешёвая замена молчанию.
 *
 * Обратно (обычный ДЦ берёт домен медийного) не наследуем: медийная запись появляется
 * только у ДЦ2, и распространять её на всех значило бы решать за подбор. */
static const char *route_domain(int dc, int media) {
    int i = route_idx(dc, media);
    if (i >= 0 && g_route[i].how == RT_WS && g_route[i].domain[0])
        return g_route[i].domain;
    /* Только при МОЛЧАНИИ карты (RT_AUTO). Запись `2m direct` — это ответ подбора, а не
     * пробел: медийный ДЦ2 измерен и признан доступным напрямую, и подставлять ему домен
     * соседа значило бы отменять замер догадкой. */
    if (media && i >= 0 && g_route[i].how == RT_AUTO) {
        int j = route_idx(dc, 0);
        if (j >= 0 && g_route[j].how == RT_WS && g_route[j].domain[0])
            return g_route[j].domain;
    }
    return NULL;
}
...
int cmd_tgws_probe(int dc, int media, int direct, int timeout_s) {
    /* Номера — те, что бывают у Telegram: 1..5 и 203 (его точка у доменов пула — kws203).
     * Прежде принимались 1..9 при подсказке «1..5»: проба в ДЦ6..9 стучалась в несуществующее
     * kwsN, а 203, который мост обслуживает, проверить было нельзя вовсе. */
    if (!((dc >= 1 && dc <= 5) || dc == 203)) {
        fprintf(stderr, LOG_W "номер ДЦ: 1..5 или 203\n");
        return 2;
    }
