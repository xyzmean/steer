...
/* Опции КАЖДОГО нашего SYN — первого и повторов. Одно имя на оба места, потому что повтор
 * шёл с OBFS_OPT_MSS: потеряй сеть первый SYN (обычная причина повтора), и conntrack по дороге
 * видел SYN без масштаба окна — сессия жила с окном 64 КиБ, то есть ровно с той скоростью,
 * ради которой масштаб и вводили; заодно повтор отличался от первого по отпечатку. */
#define OBFS_SYN_OPTS  OBFS_OPT_SCALE
...
/* OBFS_CSUM_MAX — до какого размера пакета осмысленно проверять контрольную сумму TCP.
 *
 * Всё, что длиннее, собрано GRO нашего же ядра: длину в заголовке IP оно обновляет, а поле суммы
 * оставляет от первого сегмента, и сумма заведомо не сходится (см. obfs_parse). Из сети пакет
 * такого размера прийти не может — его не пронесёт путь с MTU 1500. */
#define OBFS_CSUM_MAX 1600
 ...
 *
 * Объявлены наружу ради xsteer (src/proto/xsteer/xsclient.c и xshub.c): он несёт свой протокол
...
/* МЕТКА СОКЕТА НАВЕРХ — для `via` (см. «вложенные выходы» в spec.h).
 *
 * Клиенту (обфускатор, xsteer) её называют один раз при старте: out_underlay_mark выхода.
 * Дальше obfs_raw_open ставит её каждому своему сырому сокету ДО connect() — именно connect()
 * выбирает маршрут, а с ним устройство и адрес источника для контрольной суммы, — а
 * obfs_mark_sock ставит её сокету, который клиент открывает сам (поток xsteer по настоящему
 * TCP). Ноль — не метить вовсе, прежнее поведение.
 *
 * required — метка обязательна (задан `via`): тогда отказ SO_MARK становится отказом
 * соединения. Иначе туннель молча ушёл бы мимо выхода-цели напрямую — ровно туда, куда via
 * его не пускал. Необязательная метка (STEER_SELF_MARK на телефоне без via) лишь помогает
 * узнать трафик движка, и её отказ соединения не рвёт.
 *
 * Серверная половина (obfs_raw_open_from у сервера обфускации и хаба) метку не ставит: у неё
 * нет выхода и нет via. */
void obfs_set_sock_mark(uint32_t mark, int required);
/* 0 — метка встала или не требуется; -1 — обязательная метка не встала (errno от setsockopt). */
