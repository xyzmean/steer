...
{"xsteer", "Звезда xsteer", "[<выход>]",
 ...
 "--spec --state-dir --config --device", 0, 0, 1, 0, 1},
...
{"xsteer-check", "Звезда xsteer", "",
 ...
 "Читает конфигурацию в стиле wg и молчит, если она годна; иначе печатает, что именно не\n"
 "так, с номером строки, и отвечает кодом 2. Нужна прослойке netifd: она проверяет\n"
