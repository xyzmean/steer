...
static const struct cli_flag FLAGS[] = {
    ...
    {"--kind",      NULL, "ВИД",        "только выходы этого вида: interface, vless, direct"},
...
{"outputs", "Диагностика", "",
 ...
 "--spec --state-dir --kind --obfs", 0, 0, 0},
...
{"vless-probe", "VLESS", "<выход>",
 ...
 "--spec --state-dir --node --timeout", 1, 0, 1},
...
static void help_all(FILE *out) {
    ...
    for (size_t i = 0; i < CMDS_N; i++) {
        ...
        if (CMDS[i].ext) fputs(" [steer-extended]", out);
#endif
        fputc('\n', out);
    }
    fputs("\nСправка и версия:\n"
...
void cli_parse(const struct cli_cmd *cmd, int argc, char **argv, int from,
    ...
    for (int i = from; i < argc; i++) {
        ...
        if (a[0] != '-' || !a[1]) {
            ...
            if (out->npos >= cmd->npos || out->npos >= CLI_MAXPOS) {
                if (cmd->npos == 0)
