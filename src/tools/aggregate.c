...
static int parse_line(const char *line, uint32_t *lo, uint32_t *hi) {
    ...
    if (*e == '/') {
        ...
        /* Косая без цифр — не /0, а обрезанная строка. strtol на пустом хвосте молча даёт
         * 0, и «10.0.2.0/» (типичный хвост недокачанного файла) становился 0.0.0.0/0 —
         * весь IPv4 одной строкой, к тому же законной по форме: проверка списка в
         * splify2-update-lists идёт ПОСЛЕ подгонки и такую строку пропускает. Хвост после
         * длины («/24x») — тот же брак. Длину, обрезанную до «/2», отсюда не отличить от
         * настоящей /2 — это забота того, кто скачивает. */
        if (endp == e + 1) return -1;
        if (*endp && *endp != '\n' && *endp != '\r' && *endp != ' ' && *endp != '\t') return -1;
 ...
 * причина ровно та, которую src/compile/print.c уже назвал у emit_elements («fputs, а не
...
/* Флаги фиттера печатает он сам, а не таблица в src/cli/cli.c: у fit свой разбор
