...
void load_spec(const char *path) {
    ...
    for (size_t i = 0; i < g_ch_n; i++) {
        ...
        /* Канал `any` в туннель уводит ВЕСЬ трафик клиентов, и без явного признака это
         * почти всегда описка. У правила НА УСТРОЙСТВО согласия не требуется: запрет заведён
         * против «все клиенты без интернета и починка с провода», а здесь цена ошибки — один
         * хозяин, и он же её и заметит. Ровно эта пара («весь трафик телефона в туннель» и
         * «этот ноутбук не маршрутизируем») и просилась. */
