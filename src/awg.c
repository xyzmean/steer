...
/* Поле `via` у выхода разбирает и превращает в правила ветка via (вложенная маршрутизация
 * для всех видов выходов); в этой ветке его ещё нет. Всё, что awg нужно от via, — имя выхода,
 * через который пускать UDP туннеля, и спрашивается оно ЗДЕСЬ, одним местом: при слиянии
 * функция возвращает поле выхода (o->via, когда оно не пустое), и больше ничего менять не
 * придётся. */
 ...
 *
 * Без via — метка «мимо каналов движка». На телефоне это STEER_SELF_MARK, значение, которым
 * резолвер метит свой запрос наверх: правила маршрутизации на него нет (пакет идёт по
 * лестнице netd, младшие 16 бит нулевые — правило сети по умолчанию), а заворот DNS его
 * пропускает. Правила каналов на телефоне (self, uid:N) сокет туннеля не ловят и без метки:
 * сокет у WireGuard ядерный, без файла, и `meta skuid` на нём не совпадает, — но собственная
 * метка движка делает это свойством раскладки, а не удачей, и в `steer conns` такие записи не
 * выдаются за трафик приложений (тот же признак STEER_SELF_MARK). На роутере — 0: свой трафик
 * роутера движок не метит вовсе (разметка стоит в prerouting по адресу клиента), и нулевая
 * метка — ровно то, чем ходит всё остальное, что роутер порождает сам.
 *
 * Выход via с нулевой меткой (kind=direct) означает то же, что via нет. */
int awg_sock_mark(const char *via, uint32_t *mark) {
    ...
    *mark = self;
    if (!via || !*via) return 0;
    const struct output *v = out_by_name(via);
    if (!v) return -1;
