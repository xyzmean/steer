...
static int ms_feed(struct msgsplit *m, struct upstream *u,
    ...
    while (off < n) {
        /* Сколько байт текущего куска УЖЕ разобрано. Заведено до отказа шифра нарочно: на
         * отступление можно попасть и оттуда, а «остаток потока» обязан быть определён на
         * любой дорожке, ведущей к give_up. */
        size_t i = 0;
...
give_up:
         ...
         *
         * ОСТАТОК СЧИТАЕТСЯ ОТ off + i, А НЕ ОТ off. Первые i байт этого куска уже разобраны
         * и уже ушли наверх — кто нарезанными кадрами через накопитель, кто префиксом hc
         * строкой ниже. Отсчёт от начала куска слал бы их ВТОРОЙ РАЗ, и отступление,
         * заведённое ради того, чтобы соединение человека пережило непонятный префикс,
         * ломало бы его наверняка: сервер получает повтор, гамма обфускации у него уезжает,
         * и дальше не расшифровывается ничего. */
        m->on = 0;
        ...
        m->hdr_n = 0;
        {
            size_t done = off + i;
            size_t left = n - done;
            return left ? ws_send(u, p + done, left) : 0;
        }
    }
    return mo_flush(&w);
}
