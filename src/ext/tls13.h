...
/* То же рукопожатие, но с ПРОВЕРКОЙ ПОДЛИННОСТИ СЕРВЕРА по сертификату.
 *
 * Нужно ровно security=tls. У Reality подлинность доказана иначе (аутентификатор в
 * session_id, см. reality.c), и там проверять цепочку не только незачем, но и нельзя:
 * сертификат принадлежит чужому маскировочному сайту. У обычного TLS другого доказательства
 * нет вовсе, поэтому здесь оно обязательно — и обязательно ЦЕЛИКОМ: цепочка до корня плюс
 * подпись CertificateVerify над транскриптом.
 *
 * host  — имя, которое обязано найтись в сертификате (оно же уехало в SNI);
 * roots — путь к хранилищу корней, NULL/"" для умолчания.
 *
 * Отдельной функцией, а не флагом в старой: у неё три вызывающих, и двум из них
 * (tgws, xsteer) проверка не нужна ни при каких настройках. Признак, который у двух
 * вызывающих из трёх всегда выключен, — это не признак, а вторая функция. */
