 ...
 *
 * Было 2048, и этого перестало хватать. Reality с постквантовым обменом (Xray-core 25.9+)
 * кладёт в ссылку параметр `pqv` — ключ ML-KEM целиком, в base64url. Снято на живой
 * подписке: `pqv` в 2603 знака, вся ссылка — 2860 байт. Подписка из одной такой ссылки
 * давала «пригодных узлов: 0» и объяснение «ссылка длиннее 2048 байт», то есть выход
 * собрать было не из чего, притом что сама подписка скачивалась и была верна.
 *
 * 8192, а не 4096: запас взят на вырост ключа (ML-KEM-1024 длиннее 768-го), а буфер живёт
 * на стеке ОДНОЙ подкоманды CLI, рядом с которой уже стоят два статических буфера по
 * 256 КБ под текст подписки, — восемь килобайт здесь ничего не решают. Разбор в рабочих
 * потоках туннеля (стек 128 КБ) этот путь не проходит. */
