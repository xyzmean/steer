 ...
 * сервер дополнительно подписывает свой временный сертификат (transport/internet/reality/
 * reality.go в Xray: подпись лежит в расширении сертификата, а проверяется ML-DSA-65 по
 * этому ключу). Мы её пока не проверяем, поэтому параметр только разбирается и хранится.
 *
