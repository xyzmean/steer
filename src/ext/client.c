...
static int tcp_connect(const char *host, uint16_t port, int timeout_s) {
    ...
    /* Приёмный буфер побольше — это НЕ «на всякий случай», а развязка между сервером и
     * клиентом. Пока мы отдаём клиенту прочитанное, читать у сервера мы не можем; если
     * буфера мало, у сервера закрывается окно, и заново открыть его стоит круга до
     * сервера (десятки миллисекунд). Тогда скорость определяется не полосой, а этой
     * задержкой: замерено 3 Мбит/с там, где криптография держит 800.
     *
     * Полмегабайта — это ~40 мс полосы в 100 Мбит/с, чего хватает, чтобы сервер не
     * останавливался, пока мы возимся с клиентом. Ядро может выдать меньше (лимит
     * net.core.rmem_max), и это не ошибка: просто развязка будет короче. */
    int rcv = 512 * 1024;
