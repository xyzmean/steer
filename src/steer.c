...
static int cmd_diag(const char *spec) {
    ...
    /* 6b. Публичный резолвер внутри списка канала на выходе VLESS. Продолжение проверки 6,
     *     но с другим приговором: там ограничение названо в общем виде, здесь оно уже
     *     сработало в конкретной настройке, и цена — не «QUIC не пойдёт», а «DNS не
     *     работает вовсе».
     *
     *     Механика. Метка ставится по `ip daddr @набор` без разбора протокола, поэтому
     *     UDP-запрос к адресу из списка уходит в туннель наравне с TCP, а туннель отвечает
     *     на UDP отказом (проверка 6). Клиент, у которого 8.8.8.8 прописан руками, теряет
     *     не часть трафика, а способность разрешать имена: браузеру тут не на что
     *     откатиться, в отличие от QUIC → TCP.
     *
     *     Приговор зависит от того, есть ли перенаправление DNS: оно генерируется только
     *     при доменных правилах (has_domains), и тогда запрос клиента из from_default до
     *     туннеля не доходит — его перехватывает свой резолвер. Без доменных правил
     *     перенаправления нет вовсе, и тогда это не риск, а поломка. Постоянного жёлтого
     *     из этого не выходит: условие снимается уборкой одной категории из списка. */
    for (size_t i = 0; i < g_grp_n; i++) {
        struct output *o = out_by_name(g_grp[i].out);
        if (!o || o->kind != OUT_VLESS) continue;
        char found[64];
        ...
        if (has_domains())
            snprintf(why, sizeof(why),
                     "UDP к резолверу уйдёт в туннель, а туннель UDP не несёт. Клиентов из "
                     "from_default прикрывает перенаправление DNS, остальных — нет: у них "
                     "имена перестанут разрешаться");
        else
            snprintf(why, sizeof(why),
                     "UDP к резолверу уйдёт в туннель, а туннель UDP не несёт. Перехватить "
                     "запрос нечем: доменных правил нет, значит нет и перенаправления DNS. "
                     "Уберите из списка канала категорию с адресами резолвера");
        diag("resolver", has_domains() ? "note" : "warn", what, why);
