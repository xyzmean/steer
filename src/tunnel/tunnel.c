...
/* Простой, после которого поток DNS уступает место в ПОЛНОЙ таблице (см. conn_new). Больше
 * срока ожидания ответа у резолверов (glibc — 5 с на попытку), чтобы не вытеснить запрос,
 * ответ на который ещё едет. */
...
/* Растянуть кольцо, если «в пути» упирается в ЕГО размер, а не в приёмник.
 *
 * Запас под целую порцию вычитается из ёмкости кольца, и при начальных 32 КБ на «в пути»
 * оставалось 32768 − 19472 = 13296 байт. Пока порции были записями TLS по 8 КБ, это
 * означало «полторы записи в пути» и половину витков цикла в ожидании окна. В режиме
 * прямого копирования одно чтение приносит целый TUNNEL_BUF — 18448 байт, то есть БОЛЬШЕ
 * остатка, — и второй порции в пути не бывало никогда: замерено «в пути до 18448» при
 * окне клиента 512 КБ, то есть предел ставили мы сами, а не клиент.
 *
 * Прежде кольцо росло только в emit_to_client — когда очередная порция в него не влезала.
 * Такой рост не наступал вовсе: порция в 8-18 КБ в свободные 19 КБ влезает всегда, и
 * ёмкость навсегда оставалась начальной.
 *
 * Ленивость выделения при этом сохраняется полностью: сюда мы попадаем, только когда по
 * соединению уже идут данные и клиент объявил окно больше нашего кольца. Простаивающее
 * соединение — то, ради которого RTX_START и введён, — остаётся с малым кольцом. */
static void rtx_widen_if_self_bound(struct conn *c) {
    /* Кольца ещё нет — значит данных по соединению не было ни байта, и растить нечего:
     * client_room в этом состоянии и так отвечает по RTX_START. Без этой проверки
     * соединение с малым НАЧАЛЬНЫМ окном клиента (а 14600 объявляет половина стеков)
     * получало бы полные 64 КБ ещё до первой порции — то есть ровно ту жадность, от
     * которой RTX_START и избавил. */
    if (!c->rtx.cap) return;
    if (c->rtx.cap >= RTX_CAP) return;
    /* Упор в клиента, а не в нас: расти незачем, окно всё равно не откроется. */
 ...
 *
 * Не const: обнаружив, что мешает размер кольца, мы его тут же и растягиваем — иначе
 * проверка вечно отвечала бы «нет» по причине, которую сама же и создаёт. */
 ...
 * мегабайт: второе ядро ушло бы на борьбу за первое. Поэтому таблица потоковая — и поэтому
 * же потоков по умолчанию ОДИН, см. worker_count.
 *
 * ЗДЕСЬ СТОЯЛО, что ядро раскладывает пакеты по очередям устройства, хэшируя поток
 * симметрично, поэтому обе половины одного TCP-соединения всегда попадают одному потоку, —
 * со ссылкой на tun_open. Утверждение неверное, и ссылка ведёт в место, где ничего такого
 * не устанавливается: tun_open только открывает N очередей с IFF_MULTI_QUEUE, никакой
 * программы распределения (TUNSETSTEERINGEBPF) не ставит и вообще про распределение не
 * знает. Что делает ядро — его дело, и оно эту гарантию не даёт.
 *
 * ЧЕМ ЭТО ОБХОДИЛОСЬ. Половина потока, попавшая в чужую очередь, не находит своего
 * соединения: таблица-то потоковая. Подтверждения клиента не доходят до владельца
 * соединения, client_ack не двигается, client_can_take_record навсегда ложно — и владелец
 * перестаёт читать у сервера. Соединение живо в таблице и не несёт ничего.
 *
 * Замерено на роутере (Filogic, два ядра, VLESS, speedtest через туннель): при двух
 * очередях один поток держал 44 соединения при ОДНОМ витке в секунду, нуле МБ/с и 39
...
static int connq_release(struct conn *base) {
     ...
     *
     * Проверка стоит и ПОСЛЕ последнего сна: прежде она шла только перед каждым, и доклад,
     * пришедший за последние 10 мс, не спрашивался вовсе — приговор «не доложил» ставился
     * по счётчику кругов, и таблица утекала там, где освобождать было уже можно (I-193). */
    for (int spin = 0;; spin++) {
        int busy = 0;
...
static struct conn *conn_new(const struct tun_dev *tun) {
    ...
    for (int li = 0; li < g_live_n; li++) {
        ...
        /* Поток DNS (UDP на порт 53) — исключение из порога. Резолвер берёт новый исходный
         * порт на каждый запрос, поэтому такой поток — один запрос и один ответ, и через
         * DNS_IDLE_S молчания он своё сделал: ответ либо пришёл, либо клиент уже спросил
         * заново с другого порта. По общему порогу он выглядел активным все 120 с, и таблица,
         * забитая ими, отказывала новым соединениям, включая обычный TCP (I-055). Срок
         * простоя при этом НЕ меняется: исключение действует только здесь, когда свободных
         * мест нет и иначе пришлось бы отказать. */
...
static void handle_packet(const struct tun_dev *tun, const struct vless_node *node,
     ...
     *
     * Но не молча (I-321): отвечаем подтверждением ожидаемого номера. Сегмент за дырой
...
/* Сколько потоков цикла заводить, и почему по умолчанию ОДИН.
 *
 * Поток на ядро выглядит очевидной победой, и цена этого выбора была спрятана в
 * предположении, которого ядро не выполняет: что обе половины одного TCP-соединения
 * попадут в одну очередь устройства (разбор — у g_conns). Не попадают, и тогда владелец
 * соединения не видит подтверждений клиента и перестаёт читать у сервера. На роутере это
 * дало 12 Мбит против 292 на одной очереди и зависание коробки под speedtest.
 *
 * Поэтому число потоков БОЛЬШЕ ОДНОГО — это осознанное включение, а не умолчание, и
 * включать его имеет смысл только там, где распределение по очередям обеспечено чем-то
 * ещё. Ключ оставлен: без возможности вернуться на прежний путь одной переменной замер
 * «стало быстрее» нельзя перепроверить на том же железе. */
