...
| `zapret-instances` | Выходы `kind: zapret`: имя, номер очереди nfqueue, файл ключей `nfqws` — через табуляцию. Тоже для init-скрипта. |
| `steer-nfqws Q ФАЙЛ` | Отдельная команда, не подкоманда: обёртка, которую procd поднимает на каждый выход `kind: zapret`. Читает файл ключей при КАЖДОМ запуске, поэтому смена стратегии — это `/etc/init.d/steer reload_zapret`, а не перезапуск движка. |
...
сборки стоит в `at` у любого ответа — память нельзя рисовать живой. Снимок освежает и отдельный
экземпляр procd раз в пять минут; `apply` его снимает, потому что он описывает то, что было
применено до транзакции.
