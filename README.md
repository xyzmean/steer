...
без удаления настроек. Одновременно они стоять не могут — оба владеют `/usr/sbin/steerd` и
`/usr/sbin/steer`.
...
| Поле | Значение |
|---|---|
| `schema` | Версия формата, обязательно. `1` или `2`. В схеме 2 у канала есть `proto`, `ports` и `scope` (см. ниже). Незнакомое число — отказ загрузки: скомпилировать в правила фаервола настройку, которой движок не понимает, хуже, чем не применить ничего. |
...
Роутер часто служит выходной точкой не одному мосту: к домашним устройствам добавляются хосты из
Tailscale или ZeroTier, для которых он шлюз. Перечислите их интерфейсы, и те же каналы начнут
работать и для них:
...
`/32`, то есть подсеть пиров из него не вывести вовсе. Устройства, которого сейчас нет, движок не
...
- `match` — условия:
  ...
  - `srs_files` — наборы правил sing-box (`.srs`) как есть: имена из них берёт `dnsd`, подсети —
    набор nftables, сужение по протоколу и портам применяется само (см. ниже);
  ...
  - `proto` — **схема 2**: `tcp`, `udp` или `both`. Поля нет — протокол не критерий, и `both`
    значит ровно то же;
  - `ports` — **схема 2**: порты назначения, массив строк вида `"443"` или `"50000-65535"` (тире,
    как у диапазона адресов в списках). Не больше 16 записей, пересечения отвергаются.
...
как и раньше — правило без `ip daddr` безусловно, то есть накрыло бы этими портами весь интернет.

Схема поднята до 2 именно потому, что порты совпадение **сужают**. Незнакомый ключ движок
пропускает, и для расширяющих полей это правильно; здесь пропуск означал бы «сузить забыли». Поэтому
движок постарше обязан отвергнуть такую спеку целиком — что он и делает, а управляющий слой видит
код 2 при проверке через `apply --dry-run` и просит обновить движок.
...
голоса), и движок применяет его сам: одно сужение на весь список — обычный набор с `meta l4proto
...
снимается с предупреждением. Подробно — [docs/contract-v1.md](docs/contract-v1.md), «Наборы
sing-box». `steer srs-read` по-прежнему печатает набор нашим синтаксисом списка — посмотреть,
что в нём.
...
В пакете три имени. `steerd` — весь движок одним файлом: демон, компилятор, применение, помощники
выходов, резолвер и инструменты. `steer` — маленький клиент: команды, на которые отвечает демон
(`status`, `diag`, `explain`, `conns`, `dns-log`, `apply`, `reload`, `subscribe`), он посылает в
сокет демона и печатает ответ с тем же выводом и кодом возврата, что у прежней подкоманды; всё
...
`/etc/init.d/steer reload` (и прежние `reload_dnsd`, `reload_zapret`, `reapply`) просит демон
...
| `daemon [--watch] [--supervise] [--apply]` | Сам демон: сокет управления, сторож выходов, помощники и резолвер детьми, применение при старте. Протокол — [docs/ctl.md](docs/ctl.md). |
...
правило смотрит на клиента, а не на получателя. `diag` называет это (`bridge_nf`). Сам движок
...
- **Настройки переживают обновление прошивки, сам пакет — нет.** Пакет объявляет свои настройки
  системе файлом `/lib/upgrade/keep.d/steer`, поэтому `sysupgrade` и «Создать архив» в LuCI их
  берут — до этого не брали, и обновление «с сохранением настроек» оставляло подписку
  настроенной, а правила, выходы и узлы теряло молча. В списке семь путей: `spec.json`,
  `sub.txt`, `sub.userinfo`, каталог подписок `subs/`, свои списки `lists/custom/`, ключи
  `xsteer/` и стратегии выходов обхода `zapret/`. Каталог в keep.d берётся рекурсивно —
  `sysupgrade` кормит эти пути в `find`, — поэтому подкаталог объявляется одной строкой.

  Чего в списке нет намеренно: сам каталог `/etc/steer/lists` (десятки файлов, сотни
  килобайт) — отсутствующий список не мешает применению, и управляющий слой скачает его по
  расписанию. Оговорка про `lists/custom/` внутри него появилась в запуске 65 и обратна по
  смыслу: свой список набивает человек, перекачать его неоткуда, и прежнее правило теряло его
  молча вместе с чужими.
